...
{
    using Base = AttributeCellWidgetTypedBase<T, QStringList>;

public:
    AttributeCellEnum() : Base() {}

    AttributeCellEnum(const QString& name, const QString& description,
        T* attrib, const QStringList& names)
        : Base(name, description, attrib, 0, names) {
    }

    AttributeCellEnum(const QString& name, const QString& description,
        const typename Base::Getter& getter, const typename Base::Setter& setter, const QStringList& names)
        : Base(name, description, getter, setter, 0, names) {
    }

    // AttributeCellWidgetBase methods
    void refreshName(QAbstractItemModel* model, const QModelIndex& index) const override
    {
        model->setData(index, this->m_name, Qt::DisplayRole);
    }

    void refreshView(QAbstractItemModel* model, const QModelIndex& index) const override
    {
        const QStringList& names = std::get<0>(this->m_data);
        QString text = names[static_cast<int>(this->m_getter())];
    ...
    void refreshAttribute(QAbstractItemModel* model, const QModelIndex& index) const override
    {
        ...
        int id = (it == names.constEnd()) ? 0 : std::distance(names.constBegin(), it);
        this->m_setter(static_cast<T>(id));
    }
    ...
    {
        auto elemType = this->m_getter();

        QComboBox* comboBox = static_cast<QComboBox*>(editor);
        comboBox->setCurrentIndex(static_cast<int>(elemType));
    }
        ...
        const QModelIndex& index) const override
    {
        ...
        auto elemType = static_cast<T>(current);

        this->m_setter(elemType);
...
// Define the types used for vector attributes
...
// Define the main AttributeCellVector class
...
class AttributeCellVector : public AttributeCellWidgetTypedBase<
    ...
    typename AttributeCellVectorTypes<NumComponents>::type>
{
    using Base = AttributeCellWidgetTypedBase<
        typename AttributeCellVectorTypes<NumComponents>::type,
        typename AttributeCellVectorTypes<NumComponents>::type,
        typename AttributeCellVectorTypes<NumComponents>::type,
        typename AttributeCellVectorTypes<NumComponents>::type>;
...
public:
    ...
    {
        static QStringList s_labelNames = { "X", "Y", "Z", "W" };
        return s_labelNames;
    }
    ...
    static VectorType stringToVec(const QString& string)
    {
        ...
        for (int i = 0; i < NumComponents; ++i)
        {
            result[i] = parts[i].toFloat();
        }
        return result;
    }

    // Constructors
    AttributeCellVector() : Base() {}

    AttributeCellVector(const QString& name, const QString& description, VectorType* attrib, VectorType min, VectorType max, VectorType step)
        : Base(name, description, attrib, 0, min, max, step) {
    }

    AttributeCellVector(const QString& name, const QString& description, const typename Base::Getter& getter, const typename Base::Setter& setter, VectorType min, VectorType max, VectorType step)
        : Base(name, description, getter, setter, 0, min, max, step) {
    }
    ...
    void refreshView(QAbstractItemModel* model, const QModelIndex& index) const override
    {
        model->setData(index, QVariant::fromValue(vecToString(this->m_getter())), Qt::DisplayRole);
    }

    void refreshAttribute(QAbstractItemModel* model, const QModelIndex& index) const override
    {
        this->m_setter(stringToVec(qvariant_cast<QString>(model->data(index, Qt::DisplayRole))));
    }

    QWidget* createEditor(QWidget* parent, const QStyleOptionViewItem& option, const QModelIndex& index) const override
    {
        VectorType min = std::get<0>(this->m_data);
        VectorType max = std::get<1>(this->m_data);
        VectorType step = std::get<2>(this->m_data);
    ...
    void setEditorData(QWidget* editor, const QModelIndex& index) const override
    {
        VectorType value = this->m_getter();
        for (int i = 0; i < NumComponents; ++i)
        {
