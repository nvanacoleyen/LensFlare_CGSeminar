...
FlareQuad::FlareQuad(std::vector<glm::vec3> points) {
	glGenVertexArrays(1, &m_vao_quad);
	glGenBuffers(1, &m_vbo_quad);
	glGenBuffers(1, &m_ebo_quad);
	m_points = points;
...
void FlareQuad::drawQuad(glm::mat4 projection, glm::mat2x2 Ma, glm::mat2x2 Ms, glm::vec3 color, GLuint texApt, float light_angle_x, float light_angle_y, float entrancePupilHeight) {
	...
	glUniform1fv(4, 1, &light_angle_x);
	glUniform1fv(5, 1, &light_angle_y);
